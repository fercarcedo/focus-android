...
public class BrowserScreenScreenshots extends ScreenshotTest {
    ...
    @Ignore("Accessing web element in Geckoview")
    @Test
    ...
    private void takeScreenshotsOfOpenWithAndShare() throws Exception {
        ...
        UiObject openWithBtn = device.findObject(new UiSelector()
                .resourceId("org.mozilla.klar.gecko.debug:id/open_select_browser")
                .enabled(true));
        assertTrue(openWithBtn.waitForExists(waitingTime));
        openWithBtn.click();
        UiObject shareList = device.findObject(new UiSelector()
                .resourceId("org.mozilla.klar.gecko.debug:id/apps")
                .enabled(true));
        ...
        UiObject shareBtn = device.findObject(new UiSelector()
                .resourceId("org.mozilla.klar.gecko.debug:id/share")
                .enabled(true));
        device.pressBack();
        TestHelper.menuButton.perform(click());
    ...
    private void takeScreenshotOfTabsTrayAndErase() throws Exception {
        ...
        UiObject imageMenuTitle = device.findObject(new UiSelector()
                .resourceId("org.mozilla.klar.gecko.debug:id/topPanel")
                .enabled(true));
        UiObject openNewTabTitle = device.findObject(new UiSelector()
                .resourceId("org.mozilla.klar.gecko.debug:id/design_menu_item_text")
                .index(0)
                .enabled(true));
        UiObject multiTabBtn = device.findObject(new UiSelector()
                .resourceId("org.mozilla.klar.gecko.debug:id/tabs")
                .enabled(true));
        ...
        device.wait(Until.findObject(By.res("org.mozilla.klar.gecko.debug", "snackbar_text")), waitingTime);
